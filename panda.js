...
function movePandaEyes(event) {
    ...
    const maxOffset = 10; // Maximum pixels to move left or right
    const moveRatio = 0.3; // Movement ratio to control sensitivity
    ...
    eyeLeftGroup.setAttribute('transform', `translate(${offset - 5}, 2)`); // Maintain the downward look
    eyeRightGroup.setAttribute('transform', `translate(${offset - 5}, 2)`); // Maintain the downward look
}
...
function lookLeftDown() {
    ...
    eyeLeftGroup.setAttribute('transform', 'translate(-5, 2)');
    eyeRightGroup.setAttribute('transform', 'translate(-5, 2)');
